...
router.post("/admin", adminAcces, async (req, res, next) => {
  try {
    const response = await fetch("http://localhost:5000/cms/admin", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
router.get("/:guard/login", guest, async (req, res) => {
  ...
  try {
    const response = await fetch(
      `http://localhost:5000/cms/${req.params.guard}/login`
    );
    const data = await response.json();
...
router.get("/:guard/SignUP", guest, async (req, res) => {
  req.session.guard = req.params.guard;
  console.log(req.session.guard);
  const response = await fetch(`http://localhost:5000/cms/user/SignUP`);
  const data = await response.json();
  console.log(data);
...
router.get("/news", authenticate, async (req, res) => {
  try {
    const response = await fetch(`http://localhost:5000/cms/news`);
    const data = await response.json();
...
router.get("/news/:id", authenticate, async (req, res) => {
  try {
    const response = await fetch(
      `http://localhost:5000/cms/news/${req.params.id}`
    );
...
router.post("/user", adminAcces, async (req, res, next) => {
  try {
    const response = await fetch("http://localhost:5000/cms/user", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
router.post("/user/:id", adminAcces, async (req, res) => {
  try {
    const response = await fetch(
      `http://localhost:5000/cms/user/${req.params.id}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
router.get("/category/:id", authenticate, async (req, res) => {
  try {
    const response = await fetch(
      `http://localhost:5000/cms/category/${req.params.id}`
    );
